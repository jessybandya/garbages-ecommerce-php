...
// 	echo $_GET ['features'];
// }
$username = $email = $password =$password1=$errors=$name1=$name2=$id_no=$phone='';
$errors = array('email'=>'','username'=>'','password'=>'','password1'=>'','errors'=>'','name1'=>'','name2'=>'','id_no'=>'','phone'=>'');
if(isset($_POST['submit'])){
    ...
    if (empty($_POST['id_no'])) {
        $errors['id_no'] = "<p style='color:red'>ID number is required </p>";

    }else{
        $id_no = $_POST['id_no'];
        if (!preg_match('/^[0-9\s]+$/', $id_no)) {
            $errors['id_no'] = "<p style='color:red'>invalid ID number!</p>";
...
$errors['username'] = "<p style='color:red'>username already exist!</p>";
    ...
    $sql = "INSERT INTO clients (email, username, password,first_name,second_name,phone,id_no) VALUES ('$email', '$username', '$password','$name1','$name2','$phone','$id_no')";
...
<script>
    alert("Don't Forget to update your profile photo once you logged in");
</script>
</body>
</html>